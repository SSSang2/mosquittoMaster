...
*/ 
	...
	while ( (sql_row = mysql_fetch_row(sql_result)) != NULL )
	{
		...
		context = mosquitto_new("relay", true, NULL);
		...
		printf("--------------- 7 -----------------\n");		
			...
			pContext[index] = (struct mosquitto*)malloc(sizeof(struct mosquitto));
			pContext[index] = context;
		}	
		else{
			printf("out\n");
			context = pContext[index];
		}
		sprintf(topic, "NAMU/SYSTEM/PUBLISH/RELAY");
		printf("============ relay publish ===========\n");
		printf("\n\nmasterMosq -> id : %s\n", context->id);
		...
		printf("2\n");
		//mqtt3_db_message_store(db, context->id, mid, topic, 0, msg_len, message, 0, &store, 0);
		printf("3\n");
		//mqtt3_db_messages_queue(db, context->id, topic, 0, 0, &store);
		printf("4\n");
		//printf("#id : %s, sock : %d, mid : %d\n",mosq->id,  mosq->sock, mid);
		_mosquitto_send_publish(context, mid, topic, msg_len, message, 0, 0, false);
